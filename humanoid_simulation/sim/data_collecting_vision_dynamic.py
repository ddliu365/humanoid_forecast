...
if __name__ == "__main__":
    ...
    force_applied_time = np.linspace(200, 600, num=10)
    ...
    for j in range(force_position_z.size * force_magnitude.size * force_direction_angles.size * force_applied_time.size):
        gc.collect()
        # k is counter for force_direction_angles
        k = j % (force_magnitude.size *
                 force_position_z.size * force_applied_time.size) % force_direction_angles.size
        if j > 0 and j % force_direction_angles.size == 0:
            ...
            if j % (force_magnitude.size * force_direction_angles.size) == 0:
                _m = (_m + 1) % force_position_z.size  # m is counter for force_position_z
        ...
        for i in range(horizon_length):
            ...
            if i % 4 ==0:
                ...
                # cv2.imwrite(resultPath + '/pic/' + str(i//4) +'.jpg', img[2])
            ...
            if isApplyForce == True and i > force_applied_time[_n] and i < force_applied_time[_n] + 2:
        ...
        if isRecordValues == True:
            ...
            physical_data = np.vstack(
                (external_force_applied.T, j_position_arr[0:3, :], torso_orientation_arr, j_velocity_arr[0:3, :], j_velocity_arr[3:6, :], com_arr, 
                j_position_arr[7:, :], j_velocity_arr[6:, :], cop_arr[0:2, :], linear_momentum_arr[0:3, :], angular_momentum_arr[0:3, :], 
                linear_momentum_deri_arr[0:3, :], angular_momentum_deri_arr[0:3, :] ) )
            # external_force indicator, torso_position, torso_orientation, torso_translation, torso_rotation, com_position,
            # joint_position, joint_velocity, cop, linear_momentum, angular_momentum, linear_momentum_derivative, angular_momentum_derivative
            ...
            np.savetxt(resultPath + '/data.csv',
                       physical_data, delimiter=',')
            ...
            config.set('Data', 'force applied time', force_applied_time[_n])
