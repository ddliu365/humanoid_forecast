...
if __name__ == "__main__":
    ...
    for i in range (p.getNumJoints(bipd.robotId)):
    	p.setJointMotorControl2(bipd.robotId,i,p.POSITION_CONTROL,0)
    	print(p.getJointInfo(bipd.robotId,i))
    ...
    resultPath = str(os.path.abspath(os.path.join(
        bipd.pack_path, '../humanoid_simulation/result')))
    ...
    for i in range(horizon_length):
        ...
        cop_pt2=[contact_cop[0], contact_cop[1], contact_cop_force*0.01]
        color = [0, 1, 0]
        ...
        if isApplyForce == True:
            ...
            print(debug_line_force_from_ptr)
            print(forcePos)
            if i > 500 and i < 502:
                p.applyExternalForce(objectUniqueId=bipd.robotId, linkIndex=0,forceObj=force, posObj=forcePos, flags=p.LINK_FRAME)
                p.addUserDebugLine(debug_line_force_from_ptr,forcePos,[1,0,0],lifeTime=1,parentObjectUniqueId=bipd.robotId,parentLinkIndex=0)
                p.addUserDebugText(str(200),debug_line_force_from_ptr,[1,0,0],lifeTime=1,parentObjectUniqueId=bipd.robotId,parentLinkIndex=0)
            #if i >0 and i < 1000:
    ...
    print("Active force_frames:", active_contact_frames)
    print("Corresponding forces:", contact_forces)
