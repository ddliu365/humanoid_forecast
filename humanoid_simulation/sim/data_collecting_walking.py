...
if __name__ == "__main__":
    ...
    isApplyForce = True
    force_direction_angles = np.linspace(-np.pi/2, np.pi/2, num = 100)
    force_magnitude = np.linspace(300, 500, num = 100)
    force_position_z = 0
    force_apply_time = 500
    # force_position_z = np.linspace(-0.1, 0.1, num = 100)
    # force_apply_time = np.linspace(200, 600, num = 100)
    iter = 7
    iter_name = 'Force_magnitude_vs_Force_direction'
    ...
    # endregion
    for _j in range(force_magnitude.size * force_direction_angles.size):
        # j = _j + 7648
        j = _j + iter*1000
        if j == (1+iter)*1000:
            break
        ...
        k = j%(force_magnitude.size)%force_direction_angles.size # k is counter for force_direction_angles
        if j>0 and j%force_direction_angles.size == 0:
        ...
        if doPrediction:
            ...
            resultPath0 = str(os.path.abspath(os.path.join(
            bipd.pack_path, '../humanoid_simulation/data')))
            predictor.load_state_dict(torch.load(resultPath0 + '/result.pt'))
            predictor_x = []
        ...
        for i in range(horizon_length):
            ...
            if isApplyForce == True and i > force_apply_time and i < force_apply_time+2:
                ...
                print('current force magnitude:'+str(force_magnitude[l]))
                ...
                p.addUserDebugText(str(force_magnitude[l]),debug_line_force_from_ptr,[1,0,0],lifeTime=0.5,parentObjectUniqueId=bipd.robotId,parentLinkIndex=0)
