    ...
    def __init__(self, physicsClient=None, doFallSimulation=False, rendering=0):
        if physicsClient is None:
            ...
            resultPath = str(os.path.abspath(os.path.join(
                self.pack_path, '../humanoid_simulation/data')))

            if self.useRealTimeSim == 1:
                p.startStateLogging(p.STATE_LOGGING_VIDEO_MP4, os.path.join(
                    resultPath, 'biped_log.mp4'))
            ...
            if self.rendering == 1:
                ...
                self.out = cv2.VideoWriter(os.path.join(
                    resultPath, 'biped_realtime.mp4'), fourcc, 100.0, self.size)
            # endregion
...
if __name__ == "__main__":
    ...
    isApplyForce = True
    force_direction_angles = np.linspace(-np.pi/2, np.pi/2, num=1)
    force_magnitude = np.linspace(0, 200, num=1)
    force_position_z = np.linspace(-0.1, 0.1, num=1)
    ...
    for j in range(force_position_z.size * force_magnitude.size * force_direction_angles.size):
        ...
        if doPrediction:
            ...
            resultPath0 = str(os.path.abspath(os.path.join(
                bipd.pack_path, '../humanoid_simulation/data')))
            predictor.load_state_dict(torch.load(resultPath0 + '/result.pt', map_location='cpu'))
            predictor_x = []
        ...
        for i in range(horizon_length):
            ...
            if doPrediction:
                ...
                for n in range(3):
                    predictor_row.append(
                        float(angular_momentum_deri_arr[n, i]))
                for n in range(2):
            ...
            if isApplyForce == True and i > 500 and i < 521:
                ...
                force = [-155.555555555*0.8660254037844386,-155.555555555*-0.5 , 0]
                # define point in front of torso
                forcePos = [com[0], com[1], com[2] + 0.1]
